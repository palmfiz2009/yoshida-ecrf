...
# --- セッション状態の初期化 ---
if 'init_reg_v3' not in st.session_state:
    st.session_state['init_reg_v3'] = True
    defaults = {
...
with c3:
    ...
    # クラッシュ修正： key="max_size_diag_input" を追加
    st.session_state.max_size_diag = sc1.number_input("最大径 (mm)", value=st.session_state.max_size_diag, step=1.0, disabled=st.session_state.max_size_diag_unmeasurable, key="max_size_diag_input")
    st.session_state.max_size_diag_unmeasurable = sc2.checkbox("測定不能", value=st.session_state.max_size_diag_unmeasurable, key="chk_unm_diag")
...
with pc1:
    ...
    # クラッシュ修正： key="max_size_post_input" を追加
    st.session_state.max_size_post = px1.number_input("最大径 (mm)", value=st.session_state.max_size_post, step=1.0, disabled=st.session_state.max_size_post_unmeasurable, key="max_size_post_input")
    st.session_state.max_size_post_unmeasurable = px2.checkbox("測定不能", value=st.session_state.max_size_post_unmeasurable, key="chk_unm_post")
