...
# CSSは維持（省略）
st.markdown("<style>.main { background-color: #F8FAFC; } .block-container { max-width: 1050px !important; margin: auto; } h2 { font-size: 15px !important; color: #FFFFFF !important; background-color: #1E3A8A !important; padding: 10px 20px !important; border-radius: 8px !important; }</style>", unsafe_allow_html=True)
...
if cm == "cM1":
    ...
    with mc2:
        ...
        local_tx = st.selectbox("局所療法の種類*", ["選択してください", "放射線（外照射）", "放射線（定位）", "切除", "RFA・凍結など", "血管塞栓術", "その他", "該当なし"])
...
if st.button("適格性を判定する", type="primary", use_container_width=True):
    ...
    if missing:
        ...
        if evp_end and evp_start and evp_end < evp_start: reasons.append("EVP投与期間の矛盾(終了日が開始日前)")
        if diag_date and evp_start and diag_date > evp_start: reasons.append("診断日と治療開始日の矛盾")
        ...
        with c_dl1: st.download_button("📄 印刷用レポート(HTML)保存", f"<html><body><h3>JUOG レポート</h3><p>施設: {facility}<br>ID: {patient_id}<br>判定: {res_final}<br>理由: {', '.join(reasons)}</p></body></html>", file_name=f"Report_{patient_id}.html", mime="text/html")
